...
const renderCalls = `
      ...
      {renderDetailedSummaryTable('RANGE BUNGA', bungaSummary)}
      {renderDetailedSummaryTable('AGUNAN', agunanSummary)}
      {renderDetailedSummaryTable('AO', aoSummary)}
`;

// Replace `type SummaryRow` and `renderPDFSummaryTable`
code = code.replace(/type SummaryRow = \{[\s\S]*?\}\;\n\nfunction renderPDFSummaryTable[\s\S]*?<\/section>\n  \);\n\}/m, detailedTableComponent);

// Replace `buildSummary` and calculations
code = code.replace(/function buildSummary[\s\S]*?const aoSummary = buildSummary\('moName'\);/m, summaryLogic);

// Replace `{renderPDFSummaryTable...}` calls
code = code.replace(/\{renderPDFSummaryTable\('TENOR', tenorSummary\)\}[\s\S]*?\{renderPDFSummaryTable\('AO', aoSummary\)\}/m, renderCalls);

// Also remove the old `Cell` component inside `PerformaKolektibilitasPage` because we redefined it globally
code = code.replace(/const Cell = \(\{ v \}: \{ v: KolStats \}\) => \([\s\S]*?<\/>\n  \);/m, '');

fs.writeFileSync('app/performa/kolektibilitas/page.tsx', code);
console.log('Tables detailed');
