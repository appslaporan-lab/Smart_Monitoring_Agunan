...
export default function BeritaAcaraPencairanForm({ agunan }: { agunan: AgunanDetail }) {
  const [form, setForm] = useState({
    nomorDokumen: `BA-PU-${agunan.kodeRegister}-${new Date().getFullYear()}`,
    ...
    namaPihakKedua: '',
    jabatanPihakKedua: 'Kepala Kas',
    namaMengetahui: '',
    jabatanMengetahui: 'Pimpinan Cabang / Kabag Operasional',
  });
  ...
  const saveDocument = async () => {
    ...
    const response = await fetch('/api/berita-acara-pencairan', {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        keteranganPencairan: form.keteranganPencairan,
      ...
      <div className="no-print card" style={{ padding: 24, marginBottom: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
          ...
          <div>
            ...
            <input className="inputField" type="date" value={form.tanggalPencairan} onChange={(e) => updateField('tanggalPencairan', e.target.value)} />
          </div>
          <div style={{ gridColumn: '1 / -1' }}>
            <label className="label">Keterangan Pencairan</label>
            <input className="inputField" value={form.keteranganPencairan} onChange={(e) => updateField('keteranganPencairan', e.target.value)} placeholder="Contoh: Pencairan ulang untuk tambahan modal usaha" />
          </div>
        </div>
        ...
        <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: 16, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20 }}>
          <div>
            ...
            <SignaturePad label="TTD Pihak Pertama (ADM Kredit)" onChange={setSigPertama} />
          </div>
          <div>
            ...
            <SignaturePad label="TTD Pihak Kedua (Kepala Kas)" onChange={setSigKedua} />
          </div>
          <div>
            ...
            <SignaturePad label="TTD Mengetahui (Pimpinan Cabang / Kabag Operasional)" onChange={setSigMengetahui} />
          </div>
        </div>
      </div>
      ...
      <div className="formal-a4-sheet">
        ...
        <div style={{ textAlign: 'center', marginBottom: 16 }}>
          <h1 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800, textTransform: 'uppercase' }}>
            Berita Acara Serah Terima Agunan - Pencairan Ulang
          </h1>
        </div>

        <p style={{ margin: '0 0 14px', fontSize: '0.85rem', lineHeight: 1.6, textAlign: 'justify' }}>
          Pada hari ini, <strong>{formatTanggalIndo(form.tanggalSerah)}</strong>, sehubungan dengan pencairan ulang fasilitas kredit,
          telah dilakukan serah terima agunan dari Adm Kredit kepada Kepala Kas untuk keperluan proses pencairan ulang, dengan rincian sebagai berikut:
        </p>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem', marginBottom: 16 }}>
          <tbody>
            <tr>
              ...
              <td style={{ padding: '4px 8px', background: '#f1f5f9', fontWeight: 600, width: '20%' }}>Tanggal Pencairan</td>
              <td style={{ padding: '4px 8px', border: '1px solid #e2e8f0' }}>{form.tanggalPencairan ? formatTanggalIndo(form.tanggalPencairan) : '-'}</td>
            </tr>
            <tr>
            ...
            <tr>
              <td style={{ padding: '4px 8px', background: '#f1f5f9', fontWeight: 600 }}>Keterangan</td>
              <td style={{ padding: '4px 8px', border: '1px solid #e2e8f0' }} colSpan={3}>{form.keteranganPencairan || '-'}</td>
            </tr>
          </tbody>
        </table>

        <p style={{ margin: '0 0 20px', fontSize: '0.85rem', textAlign: 'justify' }}>
          Sejak ditandatanganinya berita acara ini, agunan dinyatakan digunakan kembali untuk keperluan pencairan ulang
          fasilitas kredit nasabah yang bersangkutan, dengan persetujuan pihak yang berwenang sebagaimana tercantum di bawah ini.
        </p>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
          {[
            { label: 'PIHAK PERTAMA, Menyerahkan', nama: form.namaPihakPertama, jabatan: form.jabatanPihakPertama, img: sigPertama },
            { label: 'PIHAK KEDUA, Menerima', nama: form.namaPihakKedua, jabatan: form.jabatanPihakKedua, img: sigKedua },
          ].map((p) => (
