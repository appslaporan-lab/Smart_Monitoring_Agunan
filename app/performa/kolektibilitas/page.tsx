...
export default async function PerformaKolektibilitasPage({ searchParams }: { searchParams: { periodeId?: string } }) {
  ...
  function aggregateKolektibilitas(dataRows: typeof rows) {
    ...
    for (const r of dataRows) {
      stats.total.noa++;
      stats.total.os += (r.outstanding || 0);
      if (r.kdKolektibilitas === '1') { stats.k1.noa++; stats.k1.os += (r.outstanding || 0); }
      else if (r.kdKolektibilitas === '2') { stats.k2.noa++; stats.k2.os += (r.outstanding || 0); }
      else if (r.kdKolektibilitas === '3') { stats.k3.noa++; stats.k3.os += (r.outstanding || 0); }
      else if (r.kdKolektibilitas === '4') { stats.k4.noa++; stats.k4.os += (r.outstanding || 0); }
      else if (r.kdKolektibilitas === '5' || r.kdKolektibilitas === '6') { stats.k5.noa++; stats.k5.os += (r.outstanding || 0); }
    }
    return stats;
  }
  ...
  const mapToReportRows = (dataRows: any[]) => dataRows.map((row: any) => {
        ...
        {!periodeSebelumnya ? (
          ...
          <>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right', marginBottom: 16 }}>
                ...
                <tbody>
                  {['1', '2', '3', '4', '5', 'total'].map(k => {
                     ...
                     const c = (currentStats as any)[key];
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              {['3', '4', '5'].map(k => {
                const key = `k${k}`;
                if ((currentStats as any)[key].os > (prevStats as any)[key].os || (currentStats as any)[key].noa > (prevStats as any)[key].noa) {
                  return (
