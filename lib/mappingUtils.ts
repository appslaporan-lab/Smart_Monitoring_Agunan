...
export function estimateJarakKantor(kantorString: string | null, kecamatanNasabah: string | null): number {
  ...
  if (!matchedKantorKey) return 15; // default 15 KM jika tidak ketemu
  
  // Mencari nama kecamatan di dalam matriks
  const distanceMap = DISTANCE_MATRIX[matchedKantorKey];
  for (const [namaKec, dist] of Object.entries(distanceMap)) {
